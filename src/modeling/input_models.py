...
class BasicWordEmbedLayer(torch.nn.Module):
    ...
    def forward(self, **kwargs):
        embed_args = {
            'txt_E': self.embeds(kwargs['text']).type(torch.FloatTensor),  # (B, T, E)
...
class BERTLayer(torch.nn.Module):
    ...
    """Takes in a list of tokenized strings (b_item_ids).
    Creates masks, feeds tokens + masks to BERT.
    Returns CLS, SEP and averaged embedding for each item in b_item_ids.
    """
    def apply_bert_layer(self,
                         b_item_ids):  # b_item_ids - [[tokens for string1],[tokens for string2],...], len(b_items) = B
        ...
        last_hidden_state = outputs[
            0]  # Sequence of hidden states at last layer of model - (B, max_len, hidden_size). Includes CLS, SEP and PAD token embeddings
        sep_indices_inverse = (item_ids != 102)
        item_masks = item_masks * sep_indices_inverse  # Making sep index 0 in item mask
        cls_masks = item_masks[:,
                    0]  # Will be 1 if element has CLS token, 0 otherwise. The only case where an element in batch does not have CLS is in the sentences added for padding.
        cls_masks = torch.unsqueeze(cls_masks, 1).repeat(1, self.dim)
        item_masks = item_masks[:,
                     1:]  # Removing CLS mask from item_masks. item_masks is now 1 only for tokens apart from CLS, SEP and PAD.
        token_length = item_masks.sum(
            dim=1)  # (B). No of items that are not CLS, SEP or PAD. An element in this tensor will be 0 only in case of padding sentences.
        item_masks = torch.unsqueeze(item_masks, 2).repeat(1, 1,
                                                           self.dim)  # Making item masks of dimension - (B, max_len, 768)
        ...
        last_hidden_state = last_hidden_state[:, 1:,
                            :]  # last_hidden_state now contains all embeddings except CLS token embeddings for all elements in batch
        ...
        denom[
            denom == 0] = 1  # Accounts for sentences which are added as padding as the sum of item_masks (denom) will be 0s for these sentences.
    ...
    def forward(self, **kwargs):
        b_text_ids = kwargs[
            'text']  # [[[sent1],[sent1],...] ...] if mode is sentence-level. [[text1],[text2]...] if mode is text-level. Is a tensor.
        ...
        cls_text_embeddings, averaged_text_embeddings, tokenlevel_text_embeddings, text_length = self.apply_bert_layer(
            b_text_ids)  # (B,768), (B,768), (B,max_top_len,768), (B)

        # Use BERT to get topic embedding
        cls_topic_embeddings, averaged_topic_embeddings, tokenlevel_topic_embeddings, topic_length = self.apply_bert_layer(
            b_topic_ids)  # (B,768), (B,768), (B,max_top_len,768), (B)

        embed_args = {'avg_txt_E': averaged_text_embeddings, 'avg_top_E': averaged_topic_embeddings,
                      'txt_E': tokenlevel_text_embeddings, 'top_E': tokenlevel_topic_embeddings, 'txt_l': text_length,
                      'top_l': topic_length}
        return embed_args

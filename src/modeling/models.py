...
from modeling import model_layers


class BiCondLSTMModel(torch.nn.Module):
    """
    Bidirectional Conditional Encoding LSTM (Augenstein et al, 2016, EMNLP)
    ...
    def __init__(self, **kwargs):
        ...
        self.bilstm = model_layers.BiCondLSTMLayer(
            ...
            doc_method=self.doc_m,
            premade_topic=kwargs.get('premade_topic', False)
        )
        self.dropout = nn.Dropout(p=self.drop_prob)
        self.pred_layer = model_layers.PredictionLayer(
            input_size=2 * self.num_layers * self.hidden_dim,
...
class CTSAN(torch.nn.Module):
    def __init__(self, **kwargs):
        ...
        self.ctsan_layer = model_layers.CTSANLayer(
            hidden_dim=self.hidden_dim,
            embed_dim=kwargs['embed_dim'],
        ...
        if self.sentence_level and self.sentence_v == 'default' and self.doc_method == 'topicatt':
            self.topic_att = model_layers.ScaledDotProductAttention(
                input_dim=2 * self.hidden_dim,
                use_cuda=self.use_cuda
            )

        self.lin = model_layers.TwoLayerFFNNLayer(
            input_dim=2 * self.hidden_dim,
            hidden_dim=kwargs['lin_size'],
...
class FFNN(torch.nn.Module):
    def __init__(self, **kwargs):
        ...
        self.model = nn.Sequential(
            nn.Dropout(p=kwargs['in_dropout_prob']),
            nn.Linear(in_dim, kwargs['hidden_size']),
            kwargs.get('nonlinear_fn', nn.Tanh()),
            nn.Linear(kwargs['hidden_size'], 3, bias=kwargs.get('bias', True))
        )
...
class TGANet(torch.nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.att = model_layers.ScaledDotProductAttention(self.input_dim)

        self.topic_trans = torch.empty(
            (kwargs['topic_dim'], self.input_dim),
